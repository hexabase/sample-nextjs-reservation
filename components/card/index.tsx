...
import { TimeBooking, TJob, TReservationRespond } from 'components/types/common';
...
export default function MediaCard({ reservationList }: ICardJob) {
              ...
              <Card onClick={() => handleOpen(reservation)} sx={{ maxWidth: 363, borderRadius: '20px', height: 394, cursor: 'pointer', boxShadow: '0 10px 15px rgba(0,0,0,0.04)' }}  >
                ...
                >
                  <div className='absolute top-3 left-3 text-xs font-bold text-[#fff] flex items-center gap-x-1'>
                    ...
                    {/* <p>{!reservation.date ? null : getTimeJP(reservation?.date)}</p> */}
                  </div>
                  ...
                  <div className='flex gap-[10px] flex-wrap mt-5 sm:mt-4'>
                    {reservation.time_10 == '1' ? (
                      <Button
                        ...
                        className={`h-6 rounded-[50px] text-[#fff] font-bold bg-[#BA00FF] hover:bg-[#BA00FF]`}
                      >10.00
                      </Button>
                    ) : null
                    }
