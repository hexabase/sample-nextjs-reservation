...
const RegistrationModal = ({ handleClose, handleOpen, open }: IRegistrationModal) => {
  ...
  return (
    <Modal open={open} hideBackdrop={isMobile ? true : false} onClose={() => handleClose()}>
      <div className='modal-body '>
        {bookingStep === 0 && (
          <>
            <Box sx={{ ...style, padding: '160px 60px' }} className='md:rounded-[20px]'>
              <div onClick={() => handleClose()} className='w-full text-right py-[10px]'>
                ...
                <Grid item xs={12} md={7}>
                  <div className='flex flex-col gap-[26px] text-center md:text-left'>
                    ...
                    <p className='text-xs md:text-sm'>
                      ご登録するメールアドレスをご入力後、送信ボタンをクリックしてください。
                      ご入力いただいたメールアドレス宛に届く会員登録フォームよりご登録をお願いいたします。
                    </p>
                    <div className='flex'>
                      <Formik
                        initialValues={{
                      ...
                      >
                        {({
                          ...
                          <form onSubmit={handleSubmit} className='relative md:flex w-full'>
                            <TextField
                              id='email'
                            ...
                            {touched.email && errors.email && (
                              <p className='absolute text-[#FF0000] !top-full text-xs'>
                                メールアドレスの形式が正しくありません
                              </p>
                            )}
                            <Button
                              disabled={!isValid}
                              type='submit'
                              className={`w-full md:w-auto rounded-[4px] py-2 px-8  ${
                                isValid ? 'bg-[#BA00ff] hover:bg-[#ba00ff]/[0.6]' : 'bg-[#E1E1E1]'
                              }`}
                            >
          ...
          <>
            ...
            >
              <Image
                ...
                width={208}
                height={167}
                className='inline-block mb-[40px]'
              />
