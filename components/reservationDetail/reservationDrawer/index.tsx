...
const ReservationDrawer = ({ reservationDetail, handleClose, showDrawer, imageUrl }: IReservationDrawer) => {
    ...
    <div>
      ...
      >
        ...
        <div className='flex flex-col  px-5 pt-5 pb-[134px] gap-4'>
          <Image alt='image' src={imageUrl ?? ''} width={353} height={236} className='rounded-[20px]' />
                ...
                <>
                  <div className="mt-[20px] ">
                    ...
                    <div className="flex flex-col gap-[15px] mt-5  items-center">
                      ...
                      {reservationInfo && reservationInfo?.time.map((t: any, index: any) => (
                        <Button key={index}
                          onClick={() => handleTimeSelection(t.field_id)}
                          disabled={t.value == '0'}
                          className={` w-full rounded-[50px] text-[#fff] font-bold ${t.value == '1' ? 'bg-[#BA00FF] hover:bg-[#BA00FF]' : 'bg-[#F4D8FF] hover:bg-[#BA00FF]'}`}
                        >
                      ...
                      <form onSubmit={handleSubmit}>
                        <div className='flex flex-col items-center gap-8'>
                          ...
                          <Button
                            type='submit'
                            sx={{ fontFamily: 'Noto Sans JP, sans-serif', width: '100%' }}
                            className={`text-[#fff] rounded-[50px] mt-5 py-3 ${!isValid ? 'bg-[#E1E1E1]' : 'bg-[#BA00ff] hover:bg-[#BA00ff]/[0.6]'}`}>
